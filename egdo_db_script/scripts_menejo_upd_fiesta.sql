...
SELECT @fecha:= date_add(@fecha_tomar, INTERVAL -16 DAY);/*INTERVAL = -X*/
...
SELECT @fecha:= date_add(@fecha_tomar, INTERVAL -16 DAY);/*INTERVAL = -7*/
...
SELECT @fecha_tomar:= fecha_hora FROM evento WHERE id_curso = 2 and id_actividad = 2;
SELECT @fecha:= date_add(@fecha_tomar, INTERVAL +23 DAY );/*INTERVAL = -7*/
