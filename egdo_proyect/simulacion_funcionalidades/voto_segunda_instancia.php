	...
	if($_POST){
		...
		if($resultadoVotoSegundaInstancia){
			...
			if($hayVotos > 0){
				...
				$buscarCantidadVotosSegundaInstancia = "select votos_segunda_instancia from disenio where 
				...
				if($obtenerCantidadVotosDisenio){
					...
					if($ejecutarActualizacion){
						$registrarVoto = "insert into votos values('','$user_voto_id','$id_disenio','$tipo_dis',2)";
