	...
	//Verificamos que sea una extensión permitida, si no lo es mostramos un mensaje de error
  	if(!in_array($extension, $extensiones)) {
		...
		if($tamañoArchivo1 > $tamañoMaximoBytes1) {
            ...
            $traerId="select id_actividad from actividad where nombre_actividad='fiesta'"; //traer id act
			...
			$consulta = "insert into fiesta (nombre, direccion, telefono, facebook, twitter, instagram, pagina_web, datelles_adicionales, imagen1, imagen2, id_actividad, id_usuario_propuesta) 
			...
			if($guardarRdo) {
				...
				echo '<script>
						$("#pasarimagenes input,textarea").each(function(fiesta) {
						$(this).val("");
						});
					</script>';
