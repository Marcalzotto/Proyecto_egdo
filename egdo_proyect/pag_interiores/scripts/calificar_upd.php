				...
				if($result = $conexion->query($verificarFecha)){
					if($result->num_rows > 0){
						...
						if($intervalA > 0 || $intervalM > 0 || $intervalD >=22){
										...
										if($maximos > 1){
											if($_SESSION["id_rol"] == 2){
												$insertarCalificacion = "insert into calificacion_upd(id_usuario,valor,id_lugar) values('$usuario','$valor','$lugar');";
													...
													$actualizar_calificacion = "update upd set calificacion = '$calificacion' where id_upd = '$lugar'";
								...
								$buscarSiVotoTresLugares = "select count(id_lugar) as cant from calificacion_upd where id_usuario = '$usuario'";
								if($result = $conexion->query($buscarSiVotoTresLugares)){
									$reg = $result->fetch_array(MYSQLI_ASSOC);
									$cantidad = $reg["cant"];
									if($cantidad < 3){
										$verSiVotoEseLugar = "select count(id_lugar) as cant from calificacion_upd where id_lugar = '$lugar' and id_usuario = '$usuario'";
