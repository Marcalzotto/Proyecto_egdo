...
if($_POST){
	...
	$buscarSiInsertoLugar = "select count(id_fiesta) as propuesto_cantidad from fiesta where id_usuario_propuesta = '$usuario'";	
	if($result = $conexion->query($buscarSiInsertoLugar)){
		$reg = $result->fetch_array(MYSQLI_ASSOC);
		$propuso = $reg["propuesto_cantidad"];
		if($propuso != 0){
			echo "Ya has propuesto lugar para la fiesta, no puedes volver a proponer.";
				...
				$path_foto_perfil = "lugares_fiesta/".$cod_perfil.$_FILES["file_perfil"]["name"];
				$path_foto_lugar = "lugares_fiesta/".$cod_lugar.$_FILES["file_lugar"]["name"];


		$insertLugar = "insert into fiesta(nombre,telefono,facebook,twitter,instagram,pagina_web,detalles_adicionales,foto_perfil,foto_lugar,id_actividad,id_usuario_propuesta,fecha_creacion,calle,altura,id_curso) 
		values('$nombre','$celular','$facebook','$twitter','$instagram','$web','$detalles','$path_foto_perfil','$path_foto_lugar',3,'$usuario','$fecha_creacion','$calle','$altura','$curso')";
