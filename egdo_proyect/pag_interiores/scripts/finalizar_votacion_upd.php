...
if($_POST){
	...
	if(preg_match($expReg, $num)){
		...
		$traerFecha = "select fecha_hora from evento where id_actividad = 2 and id_curso = 2";
		if($result = $conexion->query($traerFecha)){
			if($result->num_rows == 1){
				...
				if($interval->d < 15 || $interval->d >= 22){
					echo -1;
				}else{
					$dias_total = 22;
					...
					$update = "update evento set fecha_hora = '$fecha_ins' where id_actividad = 2 and id_curso = 2";
