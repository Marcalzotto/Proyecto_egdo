...
$query = $mysqli->query("SELECT total_votes, total_value, used_ips FROM galeria WHERE id='$id_sent'") or die("Error: ".$mysqli->error);
...
$check1 = $mysqli->query("SELECT used_ips FROM galeria WHERE used_ips LIKE '%".$ip."%' AND id='".$id_sent."'"); 
$voted = $check1->num_rows;
...
$newtotals = $mysqli->query("SELECT total_votes, total_value, used_ips FROM galeria WHERE id='$id_sent' ")or die(" Error: ".$mysqli->error); 
