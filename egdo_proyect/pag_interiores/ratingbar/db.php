...
$res = $mysqli->query("select total_votes, total_value, used_ips from galeria where id = '$id_sent'") or die("este Error: ".$mysqli->error);
//$query = mysql_query("SELECT total_votes, total_value, used_ips FROM galeria WHERE id='$id_sent' ") or die("este Error: ".mysql_error());
$numbers = $res->fetch_array(MYSQLI_ASSOC);
...
$check = $mysqli->query("select used_ips from galeria where used_ips like '%ip%' and id='$id_sent'") or die("este segundo error".$mysqli->error);
...
if(!$voted) {     //if the user hasn't yet voted, then vote normally...
	...
	if (($vote_sent >= 1 && $vote_sent <= $units) && ($ip == $ip_num)) { // keep votes within range
		...
		$updated = $mysqli->query($update) or die("este tercer error: ".$mysqli->error);
	} 
