...
$result = $mysqli->query("select id, total_votes, total_value, used_ips from galeria where id = '$id'") or die($mysqli->error);
//$query=mysql_query("SELECT id, total_votes, total_value, used_ips FROM galeria WHERE id='$id'")or die(" Error: ".mysql_error());
$numbers = $result->fetch_array(MYSQLI_ASSOC);
...
$tense=($count==1) ? "voto" : "votos"; //plural form votes/vote
                                      //if $count is equal to 1, so $tense take the value = "voto", if not "votos"

// determine whether the user has voted, so we know how to draw the ul/li
//$voted=mysql_num_rows(mysql_query("SELECT used_ips FROM galeria WHERE used_ips LIKE '%".$ip."%' AND id='".$id."' ")); 
$cant = $mysqli->query("select used_ips from galeria where used_ips like '%".$ip."%'");
$voted = $cant->num_rows; 
...
if ($static == 'static') {
		...
		$static_rater = array();
		$static_rater[] .= "\n".'<div class="ratingblock add-margin">';
      ...
      $rater ='';
      $rater.='<div id="este" class="ratingblock add-margin">';
